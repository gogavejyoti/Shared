(function ($) {
    $.fn.sheetConfigurator = function (options) {
        ...
        if (!$('#sheetConfigModal').length) {
            $('head').append(`
                <style id="sheetConfigModalStyles">
                    ...
                    /* dropdown search */
                    #sheetConfigModal .header-search { padding: .5rem; width: 100%; border: none; outline: none; box-sizing: border-box; }
                    #sheetConfigModal .dropdown-search-wrapper { padding: .5rem; }
                </style>
            `);
                    ...
                    <div class="modal-body">
                      <form id="sheetConfigForm" class="container-fluid">
                        ...
                        <div id="customFieldsSection" class="mt-3">
                          ...
                          <div id="customFieldsContainer" style="zoom:80%"></div>
                          <div class="form-text">Use <code>[Header Name]</code> tokens in formulas. Use the Insert button to pick headers. You can search inside Insert.</div>
                        </div>
        ...
        function buildHeaderMappingAndList(sheetData, colInput, existingMappings) {
            ...
            // NOTE: FTE Required and FTE Available removed as requested
        ...
        function createCustomFieldRow(field = { name: '', formula: '', format: 'auto', aggregate: 'none' }, availableHeaders = []) {
            ...
            const $row = $(`
                <div class="custom-field-row" data-row-id="${idx}">
                  <div class="row g-2 align-items-center">
                    ...
                    <div class="col-md-5">
                      <div class="d-flex w-100">
                        ...
                        <div class="btn-group insert-header-btn" role="group">
                          ...
                          <div class="dropdown-menu dropdown-menu-end" style="min-width:260px;padding:0;">
                            <div class="dropdown-search-wrapper"><input type="text" class="header-search" placeholder="Search headers..." /></div>
                            <div style="max-height:180px;overflow:auto;"><ul class="list-unstyled mb-0 header-insert-list" style="padding-left:0;margin:0;"></ul></div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div class="col-md-2">
                      <select class="form-select custom-format">
                        <option value="auto"${field.format === 'auto' ? ' selected' : ''}>Auto</option>
                        <option value="percent"${field.format === 'percent' ? ' selected' : ''}>Percentage</option>
                        <option value="int"${field.format === 'int' ? ' selected' : ''}>No decimal</option>
                        <option value="decimal1"${field.format === 'decimal1' ? ' selected' : ''}>1 decimal</option>
                        <option value="decimal2"${field.format === 'decimal2' ? ' selected' : ''}>2 decimals</option>
                      </select>
                    </div>
                    <div class="col-md-2">
                      <select class="form-select custom-aggregate">
                        <option value="none"${field.aggregate === 'none' ? ' selected' : ''}>None</option>
                        <option value="sum"${field.aggregate === 'sum' ? ' selected' : ''}>Sum</option>
                        <option value="average"${field.aggregate === 'average' ? ' selected' : ''}>Average</option>
                      </select>
                    </div>
                    <div class="col-md-1 text-end">
                      <button type="button" class="btn btn-sm btn-danger remove-custom-field">X</button>
                    </div>
                  </div>
                </div>
            `);
            ...
            $row.on('input', '.header-search', function (e) {
            ...
            // live validation
            $row.on('input change', '.formula-input, .custom-field-name, .custom-format, .custom-aggregate', function () {
        ...
        // Also apply formatting rules as necessary at presentation time (format param optional)
        ...
        function getFormConfig(sheetName) {
            ...
            $customFieldsContainer.find('.custom-field-row').each(function () {
                ...
                if (name && String(name).trim() !== '') {
                    cfg.customFields.push({ name: String(name).trim(), formula: formula ? String(formula).trim() : '', format, aggregate });
                }
            });
